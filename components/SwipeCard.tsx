...
import { User, Skill, UserSkill, TFunction } from '../types';
...
const SwipeCard: React.FC<SwipeCardProps> = ({ user, currentUser, onSwipe, isInteractive, allSkills, t }) => {
      ...
      <div className="relative w-full h-full bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden flex flex-col">
            ...
            <div className="absolute bottom-0 w-full p-4 bg-gradient-to-t from-black/80 to-transparent text-white">
                <div className="flex items-baseline space-x-2">
                    ...
                    {(user.totalRatings ?? 0) > 0 && (
                        <div className="flex items-center bg-yellow-400 text-black px-2 py-0.5 rounded-md" title={t('teacherRating', { rating: user.teacherRating?.toFixed(1), count: user.totalRatings })}>
                ...
                <p className="font-semibold">{t('level')} {user.level}</p>
            </div>
        </div>
        <div className="p-6 flex-grow flex flex-col justify-between">
          ...
          <div className="space-y-4">
             <div>
                <h4 className="font-semibold text-gray-700 dark:text-gray-200 mb-2 text-sm">{t('canTeach')}</h4>
            ...
            <div>
                <h4 className="font-semibold text-gray-700 dark:text-gray-200 mb-2 text-sm">{t('wantsToLearn')}</h4>
                <div className="flex flex-wrap gap-2">
