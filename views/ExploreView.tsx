...
const ExploreView: React.FC<ExploreViewProps> = ({ currentUser, usersToExplore, currentIndex, onSwipe, onGoBack }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Explore Connections</h1>
      <p className="text-lg text-gray-600 dark:text-gray-400 mb-2">Swipe right to connect, or left to pass.</p>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-8">Tip: Use Arrow Keys (← / →) or drag with your mouse.</p>
      ...
      <div className="relative w-full max-w-sm h-[600px] flex items-center justify-center">
        {cardsToDisplay.length > 0 ? (
          ...
          <div className="text-center p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-lg">
            <h2 className="text-2xl font-semibold text-gray-800 dark:text-white">That's everyone for now!</h2>
            <p className="text-gray-500 dark:text-gray-400 mt-2">Check back later for new people to connect with.</p>
          </div>
        )}
      </div>
