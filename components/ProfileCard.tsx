...
import { User, Skill, Product, Match, UserSkill, TFunction } from '../types';
...
const statusConfig = (t: TFunction) => ({
    'not-started': { text: t('statusNotStarted'), color: 'bg-gray-400', buttonText: t('startSwap'), nextStatus: 'in-progress' },
    'in-progress': { text: t('statusInProgress'), color: 'bg-yellow-500', buttonText: t('markAsComplete'), nextStatus: 'completed' },
    'completed': { text: t('statusCompleted'), color: 'bg-green-500', buttonText: null, nextStatus: null },
});
...
const InteractiveStarRating: React.FC<{ onRate: (rating: number) => void; t: TFunction }> = ({ onRate, t }) => {
...
const ProfileCard: React.FC<ProfileCardProps> = ({ user, currentUser, onConnect, matchDetails, onUpdateStatus, onSessionProposalResponse, onRateSession, allSkills, t }) => {
  ...
  const allStatusConfigs = statusConfig(t);
  const currentStatusConfig = matchDetails ? allStatusConfigs[matchDetails.status] : null;
  ...
  const handleAddToCalendar = (type: 'google' | 'ics') => {
    ...
    const event = {
        title: t('sessionWith', { name: user.name }),
        description: `A SkillSwap session.\n\n${t('youAreLearning')}: ${partnerSkill}.\n${t('youAreTeaching')}: ${userSkill}.`,
  ...
  return (
    <div className={`relative bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden border-2 border-gray-200 dark:border-gray-700 transition-transform hover:scale-105 duration-300 flex flex-col`}>
      <div className="absolute top-4 right-4 z-10">
        <button
          onClick={() => {
            if (window.confirm(t('reportUserConfirm', { name: user.name }))) {
              alert(t('reportUserAlert', { name: user.name }));
            }
          }}
          ...
          aria-label={t('reportUser', { name: user.name })}
          title={t('reportUser', { name: user.name })}
        >
      ...
      <div className="p-6 flex-grow flex flex-col">
            ...
            <div className="flex items-center">
                ...
                <div className="ml-4">
                    <div className="flex items-center">
                        ...
                        {user.linkedinUrl && (
                            <a href={user.linkedinUrl} target="_blank" rel="noopener noreferrer" title={t('linkedInProfile')} className="ml-2 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                    ...
                    <div className="flex items-center space-x-2">
                        <p className="text-sm font-semibold text-blue-500">{t('level')} {user.level}</p>
                        {user.totalRatings > 0 && (
                            <div className="flex items-center text-xs font-bold text-yellow-600 dark:text-yellow-400" title={t('teacherRating', { rating: user.teacherRating?.toFixed(1), count: user.totalRatings })}>
        ...
        <div className="mb-4">
            ...
            <p className="text-xs text-right text-gray-500 dark:text-gray-400 mt-1">{user.xp}/100 {t('xpToNextLevel')}</p>
        </div>
        ...
        <div className="space-y-4">
          <div>
            <h4 className="font-semibold text-gray-700 dark:text-gray-200 mb-2">{t('wantsToLearn')}</h4>
          ...
          <div>
            <h4 className="font-semibold text-gray-700 dark:text-gray-200 mb-2">{t('canTeach')}</h4>
            <div className="flex flex-wrap gap-2">
           ...
           <div>
            <h4 className="font-semibold text-gray-700 dark:text-gray-200 mb-2">{t('badges')}</h4>
            <div className="flex flex-wrap gap-2">
            ...
            {proposal && proposal.status === 'pending' && onSessionProposalResponse && (
                <div className="bg-yellow-100 dark:bg-yellow-900/50 p-3 rounded-lg space-y-2">
                    <p className="text-xs font-semibold text-yellow-800 dark:text-yellow-200">
                        {proposal.proposerId === currentUser.id ? t('sessionProposalAwaiting') : t('sessionProposalMessage', { name: user.name })}
                    ...
                    {proposal.proposerId !== currentUser.id && (
                        <div className="flex space-x-2 pt-1">
                            <button onClick={() => onSessionProposalResponse(user.id, 'accepted')} className="flex-1 text-xs font-bold py-1 px-2.5 rounded-md transition-colors bg-green-500 text-white hover:bg-green-600">{t('accept')}</button>
                            <button onClick={() => onSessionProposalResponse(user.id, 'declined')} className="flex-1 text-xs font-bold py-1 px-2.5 rounded-md transition-colors bg-red-500 text-white hover:bg-red-600">{t('decline')}</button>
                        </div>
                    )}
                </div>
            )}
            ...
            {matchDetails.scheduledSession && (
                 <div className="bg-gray-200 dark:bg-gray-700 px-3 py-2 rounded-md space-y-2">
                    <div className="flex items-center text-xs text-gray-600 dark:text-gray-300">
                        ...
                        <strong>{t('confirmedSession')}</strong> <span className="ml-1.5">{new Date(matchDetails.scheduledSession).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</span>
                    </div>
                     <div className="flex items-center space-x-2">
                        <button onClick={() => handleAddToCalendar('google')} className="flex-1 text-xs font-semibold py-1 px-2 rounded-md bg-white dark:bg-gray-600 hover:bg-gray-50 dark:hover:bg-gray-500 transition-colors">{t('addToGoogle')}</button>
                        <button onClick={() => handleAddToCalendar('ics')} className="flex-1 text-xs font-semibold py-1 px-2 rounded-md bg-white dark:bg-gray-600 hover:bg-gray-50 dark:hover:bg-gray-500 transition-colors">{t('downloadIcs')}</button>
                    </div>
                </div>
            )}
             {matchDetails.status === 'completed' && onRateSession && (
                <div className="bg-gray-200 dark:bg-gray-700 px-3 py-2 rounded-md">
                    <h6 className="text-xs font-semibold text-center text-gray-700 dark:text-gray-300 mb-2">{t('sessionFeedback')}</h6>
                    {matchDetails.rating ? (
                        <div className="text-center">
                            <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">{t('youRated')}</p>
                         ...
                         <div className="text-center">
                            <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">{t('rateSessionWith', { name: user.name })}</p>
                            <InteractiveStarRating onRate={(rating) => onRateSession(user.id, rating)} t={t} />
                        </div>
                    )}
                </div>
            )}
      ...
      {recommendedProducts.length > 0 && (
          <div className="bg-gray-50 dark:bg-gray-900/50 p-4 border-t border-gray-200 dark:border-gray-700">
              <h5 className="text-sm font-semibold text-gray-600 dark:text-gray-300 mb-3">{t('shopRelated')}</h5>
